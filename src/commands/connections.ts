...
function buildMenuOptions(
  ...
  if (savedConnections.length > 0) {
    ...
    for (const { serverUrl, name } of savedConnections) {
      const isActive = ctx.session.isConnectionActive(serverUrl);
      items.push({
        label: `${isActive ? "$(check)" : "$(circle-slash)"} ${name}`,
  ...
  const adHocConnections = activeConnections.filter(({ url }) => {
    return !savedConnections.some(({ serverUrl }) => serverUrl === url);
  });
  ...
  if (adHocConnections.length > 0) {
    ...
    for (const { server } of adHocConnections) {
      items.push({
        label: `$(check) ${server}`,
...
export async function connectionsMenu(ctx: CommandContext) {
  try {
    ...
    const items = buildMenuOptions(ctx, savedConnections);
    ...
    if (await handleGeneralAction(ctx, selection)) {
      return;
    }
...
async function handleConnect(ctx: CommandContext, conn: SavedConnection) {
  try {
...
async function handleDisconnect(ctx: CommandContext, conn: SavedConnection) {
...
async function handleEdit(ctx: CommandContext, conn: SavedConnection) {
  const name = await vscode.window.showInputBox({
    prompt: "Connection Name",
    value: conn.name,
  });
...
async function handleDelete(ctx: CommandContext, conn: SavedConnection) {
...
async function handleSavedConnectionAction(
  ctx: CommandContext,
  ...
  switch (detail) {
    case "connect":
      await handleConnect(ctx, conn);
      break;
    case "disconnect":
      await handleDisconnect(ctx, conn);
      break;
    case "edit":
      await handleEdit(ctx, conn);
      break;
    case "delete":
      await handleDelete(ctx, conn);
      break;
  }
}
...
async function manageSavedConnection(
  ...
  items.push(
    { label: "$(edit) Edit", detail: "edit" },
    { label: "$(trash) Delete", detail: "delete" },
  );
  ...
  await handleSavedConnectionAction(ctx, conn, selection.detail);
}
